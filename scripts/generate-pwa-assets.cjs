...
// Shield paths
const SDN_KECIL_OGOMOJOLO_SHIELD_PATH =
  'M 100,14 ' +
  'C 90,14 80,18 72,24 ' +
  'C 60,18 48,18 36,26 ' +
  'C 24,35 20,50 20,70 ' +
...
const SDN_KECIL_OGOMOJOLO_INNER_SHIELD_PATH =
  'M 100,19 ' +
  'C 91,19 82,23 75,28 ' +
  'C 64,23 52,23 42,30 ' +
  'C 30,38 26,52 26,72 ' +
...
const schoolLogoSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 255" width="200" height="255">
  <defs>
  ...
  <path d="${SDN_KECIL_OGOMOJOLO_SHIELD_PATH}" fill="#ffffff" stroke="#0c2d64" stroke-width="3.6" stroke-linejoin="round" />
  <path d="${SDN_KECIL_OGOMOJOLO_INNER_SHIELD_PATH}" fill="none" stroke="#0284c7" stroke-width="1.4" opacity="0.9" />
  ...
  <text font-family="'Arial Black', Arial, sans-serif" font-weight="900" font-size="11" fill="#0c2d64" letter-spacing="0.8">
    <textPath href="#topNameCurve" startOffset="50%" text-anchor="middle">SDN KECIL OGOMOJOLO</textPath>
  </text>
  ...
  <g transform="translate(32, 172)">
    ...
    <path d="M 0,11 Q 68,6 136,11 L 136,22 Q 68,17 0,22 Z" fill="url(#ogoLogoRibbon)" stroke="#0c2d64" stroke-width="1.3" />
    <text x="68" y="18" font-family="'Arial Black', Arial, sans-serif" font-weight="900" font-size="8.5" fill="#ffffff" text-anchor="middle" letter-spacing="1.2">KECAMATAN PALASA</text>
  </g>
  <g transform="translate(35, 206)">
    ...
    <text x="65" y="11" font-family="Arial, sans-serif" font-weight="bold" font-size="7.4" fill="#0c2d64" text-anchor="middle" letter-spacing="0.8">KABUPATEN PARIGI MOUTONG</text>
  </g>
...
// Icon wrapped in square canvas with maroon background
function createSquareIconSvg(size, paddingRatio = 0.12) {
  const contentSize = size * (1 - paddingRatio * 2);
  const offset = size * paddingRatio;
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" width="${size}" height="${size}">
    <defs>
      <linearGradient id="bgGrad" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stop-color="#4a070e" />
        <stop offset="50%" stop-color="#3b050a" />
        <stop offset="100%" stop-color="#240205" />
      </linearGradient>
    ...
    <g transform="translate(${offset}, ${offset}) scale(${contentSize / 200}, ${contentSize / 255})" filter="url(#shadow)">
      ${schoolLogoSvg.replace('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 255" width="200" height="255">', '').replace('</svg>', '')}
    </g>
  </svg>`;
}

// Maskable icon with 18% safe-zone margin
function createMaskableIconSvg(size) {
  const paddingRatio = 0.20; // 20% safe-zone margin
  const contentSize = size * (1 - paddingRatio * 2);
  const offset = size * paddingRatio;
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" width="${size}" height="${size}">
    <defs>
    ...
    <rect width="${size}" height="${size}" fill="url(#maskBgGrad)" />
    <g transform="translate(${offset}, ${offset}) scale(${contentSize / 200}, ${contentSize / 255})">
      ${schoolLogoSvg.replace('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 255" width="200" height="255">', '').replace('</svg>', '')}
    </g>
  </svg>`;
}
...
async function run() {
  ...
  // 1. Save icon.svg
  const baseSvg = createSquareIconSvg(512, 0.12);
  fs.writeFileSync(path.join(publicDir, 'icon.svg'), baseSvg, 'utf8');
  console.log('Created public/icon.svg');

  // 2. Generate pwa-192x192.png
  const svg192 = Buffer.from(createSquareIconSvg(192, 0.10));
  await sharp(svg192).png().toFile(path.join(publicDir, 'pwa-192x192.png'));
  console.log('Created public/pwa-192x192.png');

  // 3. Generate pwa-512x512.png
  const svg512 = Buffer.from(createSquareIconSvg(512, 0.10));
  ...
  await sharp(maskableSvg).png().toFile(path.join(publicDir, 'pwa-maskable-512x512.png'));
  console.log('Created public/pwa-maskable-512x512.png');

  // 5. Generate apple-touch-icon.png (180x180)
  const appleSvg = Buffer.from(createSquareIconSvg(180, 0.12));
  await sharp(appleSvg).png().toFile(path.join(publicDir, 'apple-touch-icon.png'));
  console.log('Created public/apple-touch-icon.png');

  // 6. Generate favicon.ico / favicon.png (64x64)
  const faviconSvg = Buffer.from(createSquareIconSvg(64, 0.08));
